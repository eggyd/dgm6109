...
//  Add third property: stress level (1-5)
let FANG_stress = [
  { x: 28, y: 4, stress: 1 },
  { x: 75, y: 4, stress: 2 },
  { x: 90, y: 4, stress: 1 },
...
// Stress level color scale
let stressColors = ["#add8e6", "#87ceeb", "#4682b4", "#4169e1", "#00008b"]; 
// 1 less stress=lightblue, 5 higher stress=darkblue 


/*  Draw scatterplot */
let circles = svg.selectAll("circle") // Select all circles (none yet//
    .data(FANG_stress) // Bind the data//
    .join("circle"); //new circle//
...
circles
    ...
    .attr("fill", function (d) {
        // fill color based on stress 
        return stressColors[d.stress - 1]; // stress 1–5 → index 0–4
    })
    .attr("opacity", 0.9); 
...
svg.append("text")
    ...
    .text(d3.max(FANG_stress, function (d) { return d.x; }) + " min");
...
//  color key 
for (let i = 1; i <= 5; i++) {  // loop 1–5 
    svg.append("circle") 
        .attr("r", 6)
        .attr("cx", svgWidth - margin - 80) 
        .attr("cy", svgHeight - margin - 110 + i * 20) 
        .attr("fill", stressColors[i - 1]);

    svg.append("text") 
        .text("Stress " + i)
        .attr("x", svgWidth - margin - 65)
        .attr("y", svgHeight - margin - 110 + i * 20)
        .attr("font-size", "11px");
}
// title for key
svg.append("text")
